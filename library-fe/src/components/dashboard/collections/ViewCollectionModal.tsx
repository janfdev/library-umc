import { Book, User, Info, Calendar, Hash, Layers } from "lucide-react";
...
const DetailRow = ({ icon: Icon, label, value, colorClass = "text-slate-600" }: any) => (
  <div className="flex items-start gap-4 p-4 rounded-2xl bg-slate-50/50 border border-slate-100/50">
    ...
    <div>
      ...
      <p className={`text-[14px] font-bold mt-0.5 ${colorClass}`}>{value || "-"}</p>
    </div>
  </div>
);

export default function ViewCollectionModal({ isOpen, onClose, collection }: ViewCollectionModalProps) {
  if (!collection) return null;

  const stockValue = Number(collection.stock) || 0;
  const statusInfo = stockValue === 0 
    ? { bg: "bg-orange-50", text: "text-orange-500", label: "Dipinjam / Stok Habis", iconColor: "text-orange-400" }
    : { bg: "bg-emerald-50", text: "text-emerald-600", label: "Tersedia", iconColor: "text-emerald-400" };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 p-1">
        ...
        <div className="lg:col-span-4">
          <div className="aspect-[3/4] rounded-[24px] overflow-hidden bg-slate-100 border border-slate-200 shadow-inner flex items-center justify-center relative group">
            ...
            <div className={`absolute top-4 left-4 px-4 py-2 rounded-full text-[11px] font-extrabold shadow-lg backdrop-blur-md ${statusInfo.bg} ${statusInfo.text} border border-white/50`}>
              {statusInfo.label}
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <DetailRow icon={User} label="Pengarang" value={collection.author} />
            <DetailRow icon={Layers} label="Penerbit" value={collection.publisher} />
            <DetailRow icon={Calendar} label="Tahun Terbit" value={collection.publicationYear} />
